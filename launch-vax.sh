...
# you should be able to get this here, but maybe not from the shell??
...
cp starunix/4.0bsd/*.init starunix/4.0bsd/boot starunix/4.0bsd/4.0bsd.tape .
...
# put the main install script on the host and run it
# dont return here until the OS is running, all packages have been installed
gcloud compute scp update-os-build-simh.sh *.init boot 40bsd.tape  ${CLOUD_USERNAME}@${INSTANCENAME}: --project ${PROJ} --zone ${CLOUDSDK_COMPUTE_ZONE}
gcloud compute ssh ${CLOUD_USERNAME}@${INSTANCENAME} --project ${PROJ} --zone ${CLOUDSDK_COMPUTE_ZONE} -- chmod +x update-os-build-simh.sh
